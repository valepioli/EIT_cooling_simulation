...
In this QuTiP simulation, time is dimensionless and normalized to the atomic decay rate (linewidth) $\gamma$. Consequently, a time value of `t = 1` corresponds exactly to one excited-state lifetime.

To convert the simulation time steps into physical time, it is necessary to account for the fact that QuTiP operates using angular frequencies (rad/s). For the $^{87}\text{Rb}$ D2 line, which has a natural linewidth of approximately **6.067 MHz**, the frequency must be multiplied by $2\pi$. 

The physical duration of a single simulation time unit is calculated as follows:
`1 / (2π × 6.067 MHz) ≈ 26.23 ns`

Therefore, **1 simulation time unit = 26.23 ns**.

#### Conversion Reference ($^{87}\text{Rb}$)

Based on this conversion factor, specific physical durations correspond to the following simulation steps (`t_total`):

* **50,000 steps** $\approx$ 1.31 ms
* **76,200 steps** $\approx$ 2.00 ms
* **190,600 steps** $\approx$ 5.00 ms
* **381,200 steps** $\approx$ 10.00 ms
* **500,000 steps** $\approx$ 13.40 ms
* **762,400 steps** $\approx$ 20.00 ms
...
The cooling time is approximately 15 ms.
