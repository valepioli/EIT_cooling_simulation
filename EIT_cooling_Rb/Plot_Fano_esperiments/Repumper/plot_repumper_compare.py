...
# --- PATH SETUP ---
# Current directory of this script (e.g., .../repumper/plot Fano experiments/)
...
def plot_all_results():
    ...
    # 1. Find all folders containing "fano_profile_rep" inside the results_fano directory
    try:
        run_folders = [f for f in os.listdir(results_fano_dir) 
                       if os.path.isdir(os.path.join(results_fano_dir, f)) and "fano_profile_rep" in f]
    ...
    if not run_folders:
        print(f"No folders named 'fano_profile_rep...' found in {results_fano_dir}.")
        return
    ...
    for idx, run_name in enumerate(run_folders):
        ...
        match = re.search(r'rep([0-9.]+)', run_name)
        rep_label = f"Repumper = {match.group(1)} $\gamma$" if match else run_name
        ...
        lines_ax1.append(l)
        labels_ax1.append(rep_label)

        # --- Plot 2: Ground State Populations ---
        ax2.plot(det_list, pop_unwanted_g2, color=color, ls="--", lw=1.5, alpha=0.8, label=f"{rep_label} (F=2 Unw.)")
        ax2.plot(det_list, pop_g1_total, color=color, ls="-", lw=2, label=f"{rep_label} (F=1 Res.)")
        ...
        ax3.plot(det_list, relative_leakage * 100, color=color, ls="-", lw=2, label=rep_label)
    ...
    ax1.legend(lines_bg + lines_ax1, labels_bg + labels_ax1, loc="upper right", fontsize='small')
    ax1.set_title("EIT / Fano Profile Comparison across Repumper Powers")
