...
def plot_eit_cooling_levels():
    ...
    # 1. DEFINE LEVELS
    # Ground F=2 is the main cooling manifold
    # Ground F=1 is the leakage manifold
    y_f1 = 0
    y_f2 = 80
    y_excited_base = 450
    y_e = {0: y_excited_base + E_e0, 1: y_excited_base + E_e1, 2: y_excited_base + E_e2, 3: y_excited_base + E_e3}

    # Draw F=1 (Leakage Sink)
    for m in range(-1, 2):
        ax.hlines(y_f1, m-0.25, m+0.25, colors='gray', lw=2, alpha=0.5)
    ax.text(2.5, y_f1, "F=1 (Leakage)", color='gray', fontweight='bold')

    # Draw F=2 (Cooling Manifold)
    for m in range(-2, 3):
        is_eit = (m == -2 or m == -1)
    ...
    # 2. IDENTIFY THE LAMBDA SYSTEM (g1, g2, e)
    # g1: Probe ground (m=-2)
    # g2: Coupler ground (m=-1)
    # e:  Common excited (m=-2)
    ...
    #  Ground (Hyperfine Splitting)
    ax.annotate('', xy=(-3.5, y_f2), xytext=(-3.5, y_f1), 
                arrowprops=dict(arrowstyle='<->', color='blue', lw=1))
    ax.text(-3.6, (y_f1+y_f2)/2, "6.834 GHz", va='center', ha='right', fontsize=10, color='blue', fontweight='bold')


    # 3. DRAW LASERS
    # PROBE (pi): g1(m=-2) -> e(m=-2)
    ax.annotate("", xy=(-2, y_e[2]-5), xytext=(-2, y_f2+5),
                arrowprops=dict(arrowstyle="->", color="blue", lw=3))
    ax.text(-2.2, (y_f2+y_e[2])/2, "Probe ($\pi$)", color='blue', rotation=90, va='center', fontweight='bold')

    # COUPLER (sigma-minus): g2(m=-1) -> e(m=-2)
    ax.annotate("", xy=(-1.95, y_e[2]-5), xytext=(-1, y_f2+5),
                arrowprops=dict(arrowstyle="->", color="green", lw=3))
    ax.text(-1.3, (y_f2+y_e[2])/2 + 50, "Coupler ($\sigma^-$)", color='green', rotation=-70, fontweight='bold')

    # REPUMPER (Recycles F=1)
    ax.annotate("", xy=(0, y_e[2]-8), xytext=(0, y_f1+5),
                arrowprops=dict(arrowstyle="->", color="orange", lw=2, ls='--'))
    ax.text(0.1, y_f1 + 150, "Repumper\n(clears leakage)", color='orange', fontweight='bold', fontsize=10)

    # 4. SHOW BLUE DETUNING (Delta)
    ax.hlines(y_e[2] + 40, -2.5, -0.5, colors='purple', ls='--', lw=2)
    ax.annotate('', xy=(-2.5, y_e[2] + 40), xytext=(-2.5, y_e[2]), 
                arrowprops=dict(arrowstyle='<->', color='purple', lw=2))
    ax.text(-3.5, y_e[2] + 20, r"$\Delta \approx 15\gamma$" + "\n(Blue Detuning)", color='purple', fontweight='bold')
