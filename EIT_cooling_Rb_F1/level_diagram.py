...
def plot_consistent_eit_diagram():
    ...
    draw_manifold(y_f1, 1, "F=1 (Coupler Ground)", active_m=-1)
    draw_manifold(y_f2, 2, "F=2 (Probe Ground)", active_m=-2) 
    ...
    ax.annotate("", xy=(-1.95, y_virtual), xytext=(-1, y_f1+5),
                arrowprops=dict(arrowstyle="->", color=c_coupler, lw=3))

    # (Repumper has been removed)
    ...
    ax.text(-1, y_f1-35, r"$|m_F=-1\rangle$", color=c_coupler, fontsize=11, ha='center', fontweight='bold')
    ...
    ax.text(-1.1, (y_f1+y_virtual)/2, "Coupling ($\sigma^-$)", color=c_coupler, fontweight='bold', ha='left')

    # Formatting
    ax.set_title("Rb87 Pure $\Lambda$-System EIT Level Diagram", fontsize=16, pad=20)
    ...
    plt.tight_layout()
    output_path = os.path.join(images_dir, "eit_diagram_lambda.png")
